...
data AccountInfo = AccountInfo
    ...
    , _aiTxHistory       :: TransactionHistory
    }
...
getTxMeta cAddr ctxId = preview $ wsAccountInfos . ix cAddr . aiTxHistory . ix ctxId

getAccountHistory :: AccountId -> Query (Maybe [CTxMeta])
getAccountHistory cAddr = toList <<$>> preview (wsAccountInfos . ix cAddr . aiTxHistory)
...
createAccount cAddr wMeta = wsAccountInfos . at cAddr ?= AccountInfo wMeta mempty mempty mempty
...
addAccountHistoryTx cAddr ctxId ctxMeta =
    wsAccountInfos . ix cAddr . aiTxHistory . at ctxId ?= ctxMeta
...
addOnlyNewTxMeta cAddr ctxId ctxMeta =
    wsAccountInfos . ix cAddr . aiTxHistory . at ctxId %= Just . fromMaybe ctxMeta
